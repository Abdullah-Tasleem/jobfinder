<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                @auth
                    @php
                        $dashboardRoute = match (Auth::user()->type) {
                            'admin' => route('admin.dashboard'),
                            'company' => route('company.dashboard'),
                            'user' => route('user.dashboard'),
                            default => url('/'),
                        };
                        $activeDashboardPattern = Auth::user()->type . '.dashboard';
                    @endphp
                @endauth
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @auth
                        ...
                        @elseif (Auth::user()->type === 'user')
                            ...
                            <x-nav-link :href="route('applied.jobs')" :active="request()->routeIs('applied.jobs')">
                                {{ __('Applied Jobs') }}
                            </x-nav-link>
                        @elseif (Auth::user()->type === 'admin')
                            <x-nav-link :href="route('admin.industries.index')" :active="request()->routeIs('admin.industries.*')">
                                {{ __('Industries') }}
                            </x-nav-link>

                            <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')">
                                {{ __('Manage Users / Companies') }}
                            </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            ...
                            @auth
                                <div>
                                    {{ Auth::user()->first_name ?? Auth::user()->company_name }}
                                </div>
                            @endauth
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @auth
                <x-responsive-nav-link :href="$dashboardRoute" :active="request()->routeIs($activeDashboardPattern)">
                    {{ __('Dashboard') }}
            ...
            <div class="px-4">
                @auth
                    <div class="font-medium text-base text-gray-800 dark:text-gray-200">
                        {{ Auth::user()->first_name ?? Auth::user()->company_name }}</div>
